    ...
    <div className="min-h-screen w-full bg-[#0d1117] text-[#e6edf3]" style={{ fontFamily: "'Geist', 'SF Pro Display', system-ui, sans-serif" }}>
      <style>{`
        ...
        @media (max-width: 640px) {
          ...
          .modules-table tbody tr.module-row { display: block; margin-bottom: 1rem; border: 1px solid #21262d; border-radius: 0.75rem; background: #0d1117; }
          .modules-table tbody td { display: flex; justify-content: space-between; align-items: center; padding: 0.75rem 1rem; border-bottom: 1px solid #21262d; text-align: right; gap: 1rem; }
          .modules-table tbody td:last-child { border-bottom: none; }
          .modules-table tbody td::before { content: attr(data-label); font-weight: 600; color: #7d8590; text-align: left; flex: 1; }
          .modules-table tbody td .action-buttons { margin-left: auto; display: flex; gap: 0.5rem; }
          /* Responsive nested days/tasks */
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-4 sm:py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-8">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <div className="relative w-full sm:w-64">
              ...
              <input type="text" placeholder="Search modules..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-[#161b22] border border-[#30363d] rounded-lg pl-9 pr-4 py-2 text-[#e6edf3] placeholder-[#484f58] focus:outline-none focus:border-[#388bfd] transition-all text-sm" />
            ...
            <button
              ...
              className="shine flex items-center justify-center gap-2 bg-[#238636] hover:bg-[#2ea043] border border-[#2ea043]/40 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-lg shadow-[#238636]/20 w-full sm:w-auto"
            >
        ...
        {/* Module Modal */}
        {showModuleModal && (
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 backdrop-blur-md p-4" onClick={() => setShowModuleModal(false)}>
            <form onSubmit={handleModuleSubmit} className="modal-enter bg-[#161b22] rounded-2xl w-full max-w-md border border-[#30363d] shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <div className="sticky top-0 bg-[#161b22] z-10 flex justify-between items-center px-4 sm:px-6 py-4 border-b border-[#21262d]">
                ...
                <button type="button" onClick={() => setShowModuleModal(false)} className="text-[#484f58] hover:text-[#7d8590] text-xl leading-none">×</button>
              </div>
        ...
        {/* Day Modal */}
        {showDayModal && (
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 backdrop-blur-md p-4" onClick={() => setShowDayModal(false)}>
            <form onSubmit={handleDaySubmit} className="modal-enter bg-[#161b22] rounded-2xl w-full max-w-md border border-[#30363d] shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <div className="sticky top-0 bg-[#161b22] z-10 flex justify-between items-center px-4 sm:px-6 py-4 border-b border-[#21262d]">
                <h3 className="text-sm font-semibold text-[#e6edf3]">{editingDay ? "Edit Day" : "New Day"}</h3>
                <button type="button" onClick={() => setShowDayModal(false)} className="text-[#484f58] hover:text-[#7d8590] text-xl leading-none">×</button>
              </div>
              <div className="px-4 sm:px-6 py-5 space-y-4">
        ...
        {/* Task Modal */}
        {showTaskModal && (
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 backdrop-blur-md p-4" onClick={() => setShowTaskModal(false)}>
            <form onSubmit={handleTaskSubmit} className="modal-enter bg-[#161b22] rounded-2xl w-full max-w-md border border-[#30363d] shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <div className="sticky top-0 bg-[#161b22] z-10 flex justify-between items-center px-4 sm:px-6 py-4 border-b border-[#21262d]">
                <h3 className="text-sm font-semibold text-[#e6edf3]">{editingTask ? "Edit Task" : "New Task"}</h3>
                <button type="button" onClick={() => setShowTaskModal(false)} className="text-[#484f58] hover:text-[#7d8590] text-xl leading-none">×</button>
              </div>
              <div className="px-4 sm:px-6 py-5 space-y-4">
          ...
          <table className="modules-table min-w-full">
            ...
            <tbody className="bg-[#0d1117] divide-y divide-[#21262d]">
              {paginatedModules.map(mod => (
                <React.Fragment key={mod.id}>
                  <tr className="module-row table-row-hover group">
                    <td data-label="Course" className="px-4 py-3 text-[#e6edf3] text-sm">{mod.course_name || mod.course?.name || "—"}</td>
                    <td data-label="Title" className="px-4 py-3 text-[#c9d1d9] text-sm break-words">{mod.title}</td>
                    <td data-label="Type" className="px-4 py-3">
                      {mod.is_common ? (
                        <span className="inline-flex bg-blue-500/10 text-blue-400 border border-blue-500/20 text-xs px-2 py-1 rounded-full">Foundation</span>
                      ) : (
                    ...
                    <td data-label="Content" className="px-4 py-3 text-[#7d8590] text-sm break-words max-w-xs">{mod.content || "—"}</td>
                    <td data-label="Actions" className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <button onClick={() => openEditModuleModal(mod)} className="p-1.5 rounded-lg text-[#7d8590] hover:text-[#388bfd] hover:bg-[#388bfd]/10" title="Edit">
                  ...
                  {expandedModuleId === mod.id && (
                    <tr>
                      <td colSpan="5" className="px-4 py-3 bg-[#0d1117]/80">
                        <div className="bg-[#161b22] rounded-lg border border-[#21262d] p-3 sm:p-4">
                          {mod.content && (
                            <div className="mb-4 pb-3 border-b border-[#21262d]">
                              ...
                              <div className="text-[#c9d1d9] text-sm whitespace-pre-wrap break-words">{mod.content}</div>
                            </div>
                          )}
                          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-3">
                            ...
                            <button
                              ...
                              className="text-xs text-[#388bfd] hover:text-blue-300 flex items-center gap-1"
                            >
                            ...
                            <div className="space-y-2">
                              {daysByModule[mod.id].map(day => (
                                <div key={day.id} className="border border-[#21262d] rounded-lg overflow-hidden">
                                  <div className="bg-[#0d1117] p-3 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 cursor-pointer hover:bg-[#161b22] day-header" onClick={() => setExpandedDayId(expandedDayId === day.id ? null : day.id)}>
                                    <div className="flex items-center gap-3 flex-wrap">
                                      {!isAdmin && (
                                        <input
                                          type="checkbox"
                                      ...
                                      <p className={`text-sm font-medium break-words ${day.is_completed ? "line-through text-[#7d8590]" : "text-[#c9d1d9]"}`}>
                                        {day.title}
                                      </p>
                                    </div>
                                  ...
                                  {expandedDayId === day.id && (
                                    <div className="bg-[#0d1117] p-3 sm:p-4 pl-4 sm:pl-8 border-t border-[#21262d]">
                                      {day.content && (
                                        <div className="mb-3 pb-2 border-b border-[#21262d]">
                                          ...
                                          <div className="text-[#c9d1d9] text-sm whitespace-pre-wrap break-words">{day.content}</div>
                                        </div>
                                      )}
                                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-2">
                                        <h5 className="text-xs font-semibold text-[#7d8590] uppercase">Tasks</h5>
                                        <button onClick={() => { setCurrentDayForTask(day.id); setEditingTask(null); resetTaskForm(); setTaskForm({ ...taskForm, day: day.id }); setShowTaskModal(true); }} className="text-xs text-[#388bfd] hover:text-blue-300 flex items-center gap-1">
                                      ...
                                      {(tasksByDay[day.id] || []).length === 0 ? (
                                        ...
                                        <div className="space-y-1.5">
                                          {tasksByDay[day.id].map(task => (
                                            <div key={task.id} className="bg-[#161b22] rounded p-2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 task-item">
                                              <div className="flex-1">
                                                <p className="text-[#c9d1d9] text-xs font-medium break-words">{task.title}</p>
                                                {task.description && <p className="text-[#484f58] text-[11px] break-words">{task.description}</p>}
              ...
              {paginatedModules.length === 0 && (
                <tr>
                  <td colSpan="5" className="text-center py-12 sm:py-20 text-[#7d8590]">No modules found</td>
                </tr>
              )}
            </tbody>
          </table>
