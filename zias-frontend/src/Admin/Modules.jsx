...
function Modules() {
  ...
  const resetModuleForm = () => setModuleForm({ course: "", title: "", content: "", is_common: true });
  ...
  return (
    <div className="min-h-screen w-screen bg-[#0d1117] text-[#e6edf3]" style={{ fontFamily: "'Geist', 'SF Pro Display', system-ui, sans-serif" }}>
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-8">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <div className="relative">
              ...
              <input type="text" placeholder="Search modules..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="bg-[#161b22] border border-[#30363d] rounded-lg pl-9 pr-4 py-2 text-[#e6edf3] placeholder-[#484f58] focus:outline-none focus:border-[#388bfd] transition-all text-sm w-full sm:w-64" />
            ...
            <button onClick={() => { setEditingModule(null); resetModuleForm(); setShowModuleModal(true); }} className="shine flex items-center justify-center gap-2 bg-[#238636] hover:bg-[#2ea043] border border-[#2ea043]/40 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-lg shadow-[#238636]/20">
        ...
        {/* Modals (already responsive, keep as is) */}
        {showModuleModal && (
        ...
        {/* Main Table with horizontal scroll on mobile */}
          ...
          <table className="min-w-full">
            <thead>
              <tr className="bg-[#161b22] border-b border-[#21262d]">
                <th className="px-3 sm:px-4 py-3 text-left text-[#7d8590] text-xs font-semibold uppercase whitespace-nowrap">Course</th>
                <th className="px-3 sm:px-4 py-3 text-left text-[#7d8590] text-xs font-semibold uppercase whitespace-nowrap">Title</th>
                <th className="px-3 sm:px-4 py-3 text-left text-[#7d8590] text-xs font-semibold uppercase whitespace-nowrap">Type</th>
                <th className="px-3 sm:px-4 py-3 text-left text-[#7d8590] text-xs font-semibold uppercase whitespace-nowrap">Content</th>
                <th className="px-3 sm:px-4 py-3 text-left text-[#7d8590] text-xs font-semibold uppercase whitespace-nowrap">Actions</th>
              </tr>
            </thead>
            <tbody className="bg-[#0d1117] divide-y divide-[#21262d]">
              {paginatedModules.map(mod => (
                <React.Fragment key={mod.id}>
                  <tr className="table-row-hover group">
                    <td className="px-3 sm:px-4 py-2.5 sm:py-3.5 text-[#e6edf3] text-xs sm:text-sm">{mod.course_name || mod.course?.name || "—"}</td>
                    <td className="px-3 sm:px-4 py-2.5 sm:py-3.5 text-[#c9d1d9] text-xs sm:text-sm">{mod.title}</td>
                    <td className="px-3 sm:px-4 py-2.5 sm:py-3.5">
                      {mod.is_common ? (
                        <span className="inline-flex bg-blue-500/10 text-blue-400 border border-blue-500/20 text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full whitespace-nowrap">Foundation</span>
                      ) : (
                        <span className="inline-flex bg-purple-500/10 text-purple-400 border border-purple-500/20 text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full whitespace-nowrap">Custom</span>
                      )}
                    </td>
                    <td className="px-3 sm:px-4 py-2.5 sm:py-3.5 text-[#7d8590] text-xs sm:text-sm truncate max-w-[120px] sm:max-w-xs">{mod.content || "—"}</td>
                    <td className="px-3 sm:px-4 py-2.5 sm:py-3.5">
                      <div className="flex items-center gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition">
                        <button onClick={() => handleEditModule(mod)} className="p-1.5 rounded-lg text-[#7d8590] hover:text-[#388bfd] hover:bg-[#388bfd]/10" title="Edit">
                  ...
                  {/* Expanded Days Row - Responsive */}
                  {expandedModuleId === mod.id && (
                    <tr>
                      <td colSpan="5" className="px-3 sm:px-4 py-3 bg-[#0d1117]/80">
                        <div className="bg-[#161b22] rounded-lg border border-[#21262d] p-3 sm:p-4">
                          ...
                          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-3">
                            ...
                            <button onClick={() => { setCurrentModuleForDay(mod.id); setEditingDay(null); resetDayForm(); setDayForm({ ...dayForm, module: mod.id }); setShowDayModal(true); }} className="text-xs text-[#388bfd] hover:text-blue-300 flex items-center gap-1">
                            ...
                            <div className="space-y-2">
                              {daysByModule[mod.id].map(day => (
                                <div key={day.id} className="border border-[#21262d] rounded-lg overflow-hidden">
                                  <div className="bg-[#0d1117] p-3 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 cursor-pointer hover:bg-[#161b22]" onClick={() => setExpandedDayId(expandedDayId === day.id ? null : day.id)}>
                                    <div className="flex items-center gap-3">
                                      ...
                                      <p className="text-[#c9d1d9] text-sm font-medium break-all">{day.title}</p>
                                    </div>
                                      ...
                                      {(tasksByDay[day.id] || []).length === 0 ? (
                                        ...
                                        <div className="space-y-1.5">
                                          {tasksByDay[day.id].map(task => (
                                            <div key={task.id} className="bg-[#161b22] rounded p-2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                                              <div className="flex-1">
                                                <p className="text-[#c9d1d9] text-xs font-medium break-all">{task.title}</p>
              ...
              {paginatedModules.length === 0 && (
                <tr><td colSpan="5" className="text-center py-16 sm:py-20 text-[#7d8590]">No modules found</td></tr>
              )}
            </tbody>
          </table>
