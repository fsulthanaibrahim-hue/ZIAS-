            ...
            <tbody className="bg-[#0d1117] divide-y divide-[#21262d]">
              {filteredStudents.map((student) => (
                <tr key={student.id} className="table-row-hover group">
                  <td data-label="Name" className="px-4 py-3 text-[#e6edf3] text-sm break-words">{student.name}</td>
                  <td data-label="Username" className="px-4 py-3 text-[#7d8590] text-sm">@{student.username}</td>
                  <td data-label="Email" className="px-4 py-3 text-[#7d8590] text-sm break-all">{student.email || "—"}</td>
                  <td data-label="Course" className="px-4 py-3 text-[#7d8590] text-sm">{student.course || "—"}</td>
                  <td data-label="Batch" className="px-4 py-3 text-[#7d8590] text-sm">{student.batch_name || student.batch || "—"}</td>
                  <td data-label="Action" className="px-4 py-3">
                    <Link to={`/admin/student-review-edit?student_id=${student.id}`} className="inline-flex items-center gap-1 bg-[#238636] hover:bg-[#2ea043] text-white px-3 py-1.5 rounded-lg text-xs font-medium shadow-md shadow-[#238636]/20">
              ...
              {filteredStudents.length === 0 && (
                <tr>
                  <td colSpan="6" className="text-center py-12 text-[#7d8590]">
