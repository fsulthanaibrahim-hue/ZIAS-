...
const StatCard = ({ label, value, icon, color }) => {
  ...
  return (
    <div className={`flex-1 min-w-[160px] rounded-xl p-5 border ${c.bg} ${c.border} flex items-center gap-4`}>
      <div className={`w-11 h-11 rounded-xl ${c.icon} flex items-center justify-center shrink-0 text-xl`}>
        {icon}
      </div>
      <div>
        <p className="text-white/55 text-xs font-semibold uppercase tracking-wider mb-0.5">{label}</p>
...
const ActionBtn = ({ label, icon, onClick, color = "blue" }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl border text-sm font-semibold transition-all ${colors[color]}`}
    >
...
function Dashboard() {
  ...
  return (
    <div className="min-h-screen w-screen bg-[#0f1623] text-white p-6 md:p-8">

      {/* Toast */}
      {notification && (
        <div className="fixed top-6 right-6 z-50 bg-violet-600/90 text-white px-5 py-3 rounded-xl shadow-2xl text-sm font-semibold backdrop-blur">
          {notification}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Bar Chart */}
        <div className="bg-[#1a2538] rounded-xl border border-white/10 p-5">
        ...
        <div className="flex flex-col gap-4">
              ...
              {[
                ...
                <div key={label} className="flex items-center justify-between text-sm">
                  ...
                  <div className="flex items-center gap-1.5">
                    <StatusDot ok={ok} />
                    <span className={ok ? "text-emerald-400 text-xs font-semibold" : "text-red-400 text-xs font-semibold"}>
                      {ok ? "Operational" : "Down"}
                    </span>
                  </div>
                </div>
          ...
          {/* Recent Activity */}
          <div className="bg-[#1a2538] rounded-xl border border-white/10 p-5 flex-1">
