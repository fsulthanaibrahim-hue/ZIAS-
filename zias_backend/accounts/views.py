    ...
    def apply_to_students(self, request, pk=None):
        try:
            ...
            if not fee_structure.is_active:
                return Response(
                    {"error": "Cannot apply inactive fee structure"},
                    status=400
                )
    ...
    def get(self, request):
        try:
            ...
            for student in students:
                ...
                total_paid = sum(float(sf.paid_amount) for sf in student_fees)
                total_pending = sum(float(sf.pending_amount) for sf in student_fees)
                
                result.append({
                    'id': student.id,
                    'name': student.full_name or f"Student {student.id}",
                    'email': student.user.email if hasattr(student, 'user') else f"student{student.id}@example.com",
                    'course': getattr(student, 'course', '—'),
                    'batch': getattr(student, 'batch', '—'),
                    'agreement_signed': getattr(student, 'agreement_signed', False),
                    'escalation_flag': getattr(student, 'escalation_flag', False),
                    'total_paid': total_paid,
                    'total_pending': total_pending,
                    'total_overdue': 0,
                    'week_back_fee_status': 'on_track'
                })
...
#         # Filter by course (only if course field exists in model)
#         course_id = self.request.query_params.get('course')
#         if course_id and hasattr(FeeStructure, 'course'):
#             queryset = queryset.filter(course_id=course_id)
        
#         # Filter by batch (only if batch field exists in model)
#         batch_id = self.request.query_params.get('batch')
#         if batch_id and hasattr(FeeStructure, 'batch'):
...
#             # Only filter by course/batch if those fields exist
#             if hasattr(fee_structure, 'course') and fee_structure.course:
#                 students = students.filter(course=fee_structure.course)
#             if hasattr(fee_structure, 'batch') and fee_structure.batch:
...
#                     "criteria": {
#                         "course": fee_structure.course.name if hasattr(fee_structure, 'course') and fee_structure.course else "Any",
#                         "batch": fee_structure.batch.name if hasattr(fee_structure, 'batch') and fee_structure.batch else "Any"
#                     }
#                 })
...
#             with transaction.atomic():
#                 for student in students:
#                     discounted_amount = fee_structure.total_amount * (1 - fee_structure.discount_percentage / 100)
...
#                         defaults={
#                             'total_amount': discounted_amount,
#                             'discount_applied': fee_structure.total_amount - discounted_amount,
...
#                 'discounted_amount': float(fee_structure.total_amount * (1 - fee_structure.discount_percentage / 100)),
...
#             total_revenue = student_fees.aggregate(Sum('total_amount'))['total_amount__sum'] or 0
#             total_collected = student_fees.aggregate(Sum('paid_amount'))['paid_amount__sum'] or 0
#             total_pending = total_revenue - total_collected
            
#             # Recent fee structures
#             recent = FeeStructure.objects.all().order_by('-id')[:5]
...
#                 'total_collected': float(total_collected),
#                 'total_pending': float(total_pending),
#                 'collection_percentage': (float(total_collected) / float(total_revenue) * 100) if total_revenue > 0 else 0,
